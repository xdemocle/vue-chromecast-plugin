...
var Sender = function () {
  ...
  _createClass(Sender, [{
    ...
    value: function initialize() {
      ...
      this.apiConfig = new chrome.cast.ApiConfig(this.sessionRequest, function () {
        return _this.sessionListener;
      }, function () {
        return _this.availabilityListener;
      });

      chrome.cast.initialize(this.apiConfig, function () {
        return _this.onInitSuccess;
      }, function () {
    ...
    value: function sessionListener(e) {
      ...
      this.session = e;
      this.session.addUpdateListener(function () {
    ...
    value: function sendMessage(message) {
      ...
      if (this.session !== null) {
        this.session.sendMessage(this.params.applicationNamespace, message, this.onSuccess.bind(this, message), this.onError);
      } else {
        chrome.cast.requestSession(function (e) {
          _this3.session = e;
          _this3.sessionListener(e);
          _this3.session.sendMessage(_this3.params.applicationNamespace, message, _this3.onSuccess.bind(_this3, message), _this3.onError);
        }, this.onError);
      }
    }
  }, {
  ...
  _createClass(Receiver, [{
    ...
    value: function initialize() {
      ...
      this.castReceiverManager.onReady = function (event) {
        _this4.log('Received Ready event: ' + JSON.stringify(event.data));
        _this4.castReceiverManager.setApplicationState(_this4.params.applicationName + ' is ready...');
      };

      this.castReceiverManager.onSenderConnected = function (event) {
        _this4.log('Received Sender Connected event: ' + event.senderId);
      };

      this.castReceiverManager.onSenderDisconnected = function (event) {
        _this4.log('Received Sender Disconnected event: ' + event.senderId);
      };
      ...
      this.messageBus.onMessage = function (event) {
        _this4.log('Message [' + event.senderId + ']: ' + event.data);

        if (event.data.method) {
          _this4.$events.$emit('message', JSON.stringify(event.data));
        }
      };
