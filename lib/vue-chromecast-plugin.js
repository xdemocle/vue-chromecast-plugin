...
exports.default = {
  install: function install(Vue, options) {
    ...
    if (_utils2.default.checkIfChromecast()) {
      _chromecastSdk.Receiver.prototype.$vueEvents = $chromecast;
      $chromecast.Receiver = new _chromecastSdk.Receiver(options);
    } else {
      _chromecastSdk.Sender.prototype.$vueEvents = $chromecast;
  ...
  _createClass(Sender, [{
    ...
    value: function stateChangedHandler(response) {
      ...
      if (response.sessionState) {
        this.$vueEvents.$emit(response.type, response.sessionState);
      }
    ...
    value: function stopCasting(callback) {
      ...
      this.$vueEvents.$emit('sessionstatechanged', 'SESSION_ENDING');
    ...
    value: function onMessageError(message) {
      if (message.code) {
        this.$vueEvents.$emit('sessionstatechanged', message.code);
      }
    ...
    value: function registerListenersCore(context) {
      ...
      context.addCustomMessageListener(this.params.appNamespace, function (customEvent) {
        ...
        if (customEvent.data.method) {
          _this4.$vueEvents.$emit('message', dataStringified);
        }
